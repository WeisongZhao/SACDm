function  out = forward_diff(data,step,dim,gpu)
...
shift(dim)=shift(dim)-1;
shiftdata2(1:shift(1),1:shift(2),1:shift(3))=data;
shiftdata1 = (shiftdata1-shiftdata2)/step;
shift(dim)=shift(dim)+1;
out = -shiftdata1(position(1):shift(1),position(2):shift(2),position(3):shift(3));
end
